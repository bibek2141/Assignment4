...
def blink_thread():
    ...
    timeout = time.time() + 60*0.30
    #assign true(change from false to true
    ledstate = not ledstate
    #run blinking mode, check timer and when yellow button is clicked Red and Green Led is turned off
    while True:
        test =0
        GPIO.output(29,True)
        time.sleep(0.5)
       ...
       #timer condition
        if test == 0.3 or time.time() >timeout:#stops at 1 minute
            break
        #When Yellow button is clicked red and green is turned off and breaks out of loop
        elif GPIO.input(BTN_Y) == ledstate:
            print("Yellow button is pressed, Red and Green Led Turns off")
            break
        test = test -1
...
def handle(pin):
    ...
    t = None
    if pin == BTN_B:
       print("Blue button is pressed please press green and red simulatenously")
